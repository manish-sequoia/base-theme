...
class Assets {
	...
	public function register_scripts() {
		...
		$this->register_script( 'base-theme-main', 'js/main.js', [ 'jquery' ] );
		$this->register_script( 'base-theme-home', 'js/home.js', [ 'base-theme-main' ] );
		$this->register_script( 'base-theme-single', 'js/single.js', [ 'base-theme-main' ] );
	...
	public function register_styles() {
		...
		$this->register_style( 'base-theme-main', 'css/main.css' );
		$this->register_style( 'base-theme-home', 'css/home.css', [ 'base-theme-main' ] );
		$this->register_style( 'base-theme-single', 'css/single.css', [ 'base-theme-main' ] );

		wp_enqueue_style( 'base-theme-main' );

		if ( is_home() ) {
			wp_enqueue_style( 'base-theme-home' );
		}

		if ( is_single() ) {
			wp_enqueue_style( 'base-theme-single' );
		}
	}
	...
	public function get_asset_meta( $file, $deps = array(), $ver = false ) {
		$asset_meta_file = sprintf( '%s/js/%s.asset.php', untrailingslashit( BASE_THEME_BUILD_DIR ), basename( $file, '.' . pathinfo( $file )['extension'] ) );
	...
	public function register_script( $handle, $file, $deps = array(), $ver = false, $in_footer = true ) {
		...
		$src        = sprintf( BASE_THEME_BUILD_URI . '/%s', $file );
	...
	public function get_file_version( $file, $ver = false ) {
		...
		$file_path = sprintf( '%s/%s', BASE_THEME_BUILD_DIR, $file );
